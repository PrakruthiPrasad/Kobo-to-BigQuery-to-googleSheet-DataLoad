...
class TestWriteToSheet:
    ...
    @patch("sheets_writer.set_with_dataframe")
    def test_append_mode_clears_and_writes_headers_when_tab_empty(
            self, mock_swdf, mock_spreadsheet, clean_df):
        """
        When a tab exists but is completely empty (e.g. created manually
        but left blank), append mode should clear it and write headers + data.
        """
        ws = MagicMock()
        ws.title = "Survey Data"
    ...
    @patch("sheets_writer.set_with_dataframe")
    def test_append_mode_clears_and_writes_headers_when_tab_has_empty_row(
            self, mock_swdf, mock_spreadsheet, clean_df):
        """
        When a tab has rows but all header cells are empty or whitespace
        treat it as empty and write headers + data from scratch.
        """
        ws = MagicMock()
        ws.title = "Survey Data"
        ws.get_all_values.return_value = [["", "", ""]]
    ...
    @patch("sheets_writer.set_with_dataframe")
    def test_append_mode_does_not_clear_when_valid_headers_exist(
            self, mock_swdf, mock_spreadsheet, clean_df):
        """Append mode must never call ws.clear() when sheet has valid headers."""
        ws = MagicMock()
        ws.title = "Survey Data"
